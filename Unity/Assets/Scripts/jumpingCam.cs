...
public class jumpingCam : MonoBehaviour
{
		...
		/*	I think this is the code for the previous information windows
		//return a string holding each line of the information
		string[] infoScreen ()
		{
				//DECIDE: read all into array or read from file when needed....
				
				//reset reading from beginning of file
				info.DiscardBufferedData ();
				info.BaseStream.Seek (0, System.IO.SeekOrigin.Begin);
				...
				while (!info.EndOfStream) {
						...
						//read until planet is found
						if (split [0].Equals (camTarget)) {
								split [0] = "";		//remove id of planet

								//split the lines at the $ sign
		...
		*/
		...
		void LateUpdate ()
		{
		Debug.Log ("JumingCam: " + Global.body[83].transform.position);
